...
class WenShu2013To2016Spider(scrapy.spiders.Spider):
    name = "wenshu_2014_spider"
    start_urls = [
    ...
    # senior_courts=[{"type":u"法院层级","name":u"最高人民法院"}]

    senior_courts=[{"type":u"法院地域", "name": u"湖南省"}]
    ...
    def parse_medium_court_area(self, response):
        ...
        try:
           menus = json.loads(response.text.replace("\\", "").strip('"'))
            ...
            error_message_list.append(u"equest参数:" + response.request.formdata)
            self.logger.error(u"\r".join(error_message_list))
...
if __name__ == "__main__":
    ...
    # 运行spider
     print scrapyd_scheduling.schedule(project="lawyer", spider=WenShu2013To2016Spider.name)
    # 取消运行spder 执行三次
    #  print scrapyd_cencel.cancel(project="lawyer",job="c1a1a19ef96611e6b3b80242c0a80002")
     pass
